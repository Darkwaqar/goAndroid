...
public class MainFragmentsAdapter extends FragmentPagerAdapter {
    ...
    private void initializeFragments() {
        ...
        mFragments.add(new TabFragment(AppConstant.COLLECTION_TAB_TITLE, HostFragment.newInstance(CollectionFragment.newInstance(mVendor))));
        if (mVendor.getMobileAppSetting().getTabAnimation() == AppConstant.BOX_ANIMATION) {
            mFragments.add(new TabFragment(AppConstant.HOME_TAB_TITLE, HostFragment.newInstance(DiscoverFragment.newInstance(mVendor))));
        } else if (mVendor.getMobileAppSetting().getTabAnimation() == AppConstant.BANNER_ANIMATION) {
            mFragments.add(new TabFragment(AppConstant.HOME_TAB_TITLE, HostFragment.newInstance(DiscoverFragmentProductsAndBanner.newInstance(mVendor))));
        } else {
            mFragments.add(new TabFragment(AppConstant.HOME_TAB_TITLE, HostFragment.newInstance(DiscoverFeatureProductListingFragment.newInstance(mVendor))));
        }
        ...
        if (mVendor.getMobileAppSetting().getAboutUsEnabled())
            mFragments.add(new TabFragment(AppConstant.ABOUT_TAB_TITLE, HostFragment.newInstance(AboutUsFragment.newInstance(mVendor))));
    }
